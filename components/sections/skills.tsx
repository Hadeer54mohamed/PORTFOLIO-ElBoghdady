...
const expertise = [
  ...
  { 
    ...
    description: "Connecting frontend with RESTful APIs and GraphQL services",
    icon: Database,
...
function SkillBar({ name, level, color, isVisible, index }: { name: string; level: number; color: string; isVisible: boolean; index: number }) {
  return (
    <div 
      className="space-y-2 opacity-0 animate-in fade-in slide-in-from-left-4 fill-mode-forwards"
            ...
            <div className="pt-6">
              ...
              <div className="flex flex-wrap gap-3">
                {tools.map((tool, index) => (
                  <div
                    key={tool.name}
                    className="flex items-center gap-2 px-4 py-2 rounded-xl bg-card border border-border hover:border-primary/50 hover:bg-primary/5 transition-all duration-300 opacity-0 animate-in fade-in fill-mode-forwards"
          ...
          <div className="space-y-8">
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              {expertise.map((item, index) => (
                <div
                  key={item.title}
                  className="group p-5 rounded-2xl bg-card border border-border hover:border-primary/30 transition-all duration-300 hover:shadow-lg hover:shadow-primary/5 opacity-0 animate-in fade-in slide-in-from-right-4 fill-mode-forwards"
